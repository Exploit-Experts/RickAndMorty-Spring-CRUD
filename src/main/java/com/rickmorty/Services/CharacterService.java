...
public class CharacterService implements CharacterServiceInterface {
    ...
    private LocationDto fetchLocationDetails(String locationUrl) {
        if (locationUrl == null || locationUrl.isEmpty()) {
            return new LocationDto(0, "Unknown", "Unknown", "Unknown", List.of(), "Unknown");
        }

        try {
            RestTemplate restTemplate = new RestTemplate();
            ResponseEntity<LocationDto> response = restTemplate.exchange(
                    locationUrl, HttpMethod.GET, null, new ParameterizedTypeReference<>() {}
            );

            LocationDto location = response.getBody();
            ...
            System.err.println("Erro ao buscar detalhes da localização: " + e.getMessage());
            return new LocationDto(0, "Unknown", "Unknown", "Unknown", List.of(), "Unknown");
        }
    }
    public CharacterDto rewriteCharacterDto(CharacterDto character) {
        LocationDto location = character.location();
